...
/usr/local/cryptsetup-dd/cryptsetup -y luksFormat $loop <<< $PPHRASE 2> /dev/null
/usr/local/cryptsetup-dd/cryptsetup luksOpen $loop $crpt <<< $PPHRASE 2> /dev/null || bad_passphrase
...
#/usr/local/cryptsetup-dd/cryptsetup luksClose $crpt
#losetup -d $loop
